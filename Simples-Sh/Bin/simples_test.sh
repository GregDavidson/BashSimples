...
for d in ${simples_sh_path:-$HOME/Lib/Sh}; do
    . $d/test_expect.sh && loaded_test_expect=true && break
done
$loaded_test_expect || (
...
case $# in
    0) script_to_test="simples.sh" ;;
    1) script_to_test="$1"; shift ;;
esac

test_main "$script_to_test" "${simples_sh:-$HOME/Lib/Sh}" "${@}"
...
var_simple_cmd echo foo5 hello world
test_var foo5 'hello world' var_simple_cmd
...
for item in $colors; do
  test_success in_simple_list "$colors" "$item"
done

test_failure in_simple_list "$colors" 'reen'
test_success ni_simple_list "$colors" 'reen'

test_output "purple $colors" simple_list_prepend "$colors" purple
test_output "$colors teal" simple_list_append "$colors" teal
...
test_output 'Hello world!' simple_load hello_world
